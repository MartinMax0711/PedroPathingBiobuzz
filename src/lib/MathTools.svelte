...
<script lang="ts">
  ...
  let labelFontSize = 12; // Label font size (px)
  ...
  <svg class="absolute top-0 left-0 w-full h-full pointer-events-none z-20">
    ...
    {#each gridPositions as position, i}
      ...
      {#if i % labelInterval === 0 || position === 0 || position === FIELD_SIZE}
        <text
          x={x(position) + (position === 0 ? 3 : position === FIELD_SIZE ? -3 : 0)}
          y={y(FIELD_SIZE) + labelFontSize + 3}
          ...
          paint-order="stroke"
          text-anchor={position === 0 ? "start" : position === FIELD_SIZE ? "end" : "middle"}
        >
          {position}"
        </text>
      {/if}
    ...
    {#each gridPositions as position, i}
      ...
      {#if (i % labelInterval === 0 || position === 0) && position !== FIELD_SIZE}
        <text
          x={x(FIELD_SIZE) - 4}
          y={y(position) + (position === 0 ? -4 : labelFontSize * 0.35)}
          font-size={labelFontSize}
          font-weight="600"
          fill="#ffffff"
          fill-opacity="0.85"
