...
<script lang="ts">
  ...
  import { shortObstacleName } from "../../utils";
  ...
  /** Obstacle overlaps of this path (robot footprint swept along it). */
  export let collisions: PathCollision[] = [];

  $: hitNames = [
    ...new Set(collisions.map((c) => shortObstacleName(c.shapeId, c.shapeName))),
  ];
  $: hitTitle = collisions.length
    ? "Robot footprint overlaps:\n" +
      collisions
        .map(
          (c) =>
            `• ${shortObstacleName(c.shapeId, c.shapeName)} (t ${c.tStart.toFixed(2)}–${c.tEnd.toFixed(2)})`,
        )
        .join("\n")
    : "";
  const hitPillClass =
    "inline-flex items-center gap-1 min-w-0 px-1.5 py-0.5 text-xs font-medium rounded whitespace-nowrap bg-rose-100 text-rose-700 dark:bg-rose-900 dark:text-rose-200";

  $: snapToGridTitle =
    $snapToGrid && $showGrid ? `Snapping to ${$gridSize} grid` : "No snapping";

  /** Clamp the typed end point to the field and record it for undo. */
  function commitEndPoint() {
    const clamp = (n: unknown) => {
      const num = Number(n);
      return Number.isFinite(num) ? Math.min(141.5, Math.max(0, num)) : 0;
    };
    line.endPoint.x = clamp(line.endPoint.x);
    line.endPoint.y = clamp(line.endPoint.y);
    lines = [...lines];
    recordChange?.();
  }
...
<div class="flex flex-col w-full justify-start items-start gap-1 rounded-md p-1">
  <!-- One row at every sidebar width: the name and chain fields give way first.
       inline-size containment keeps this row from widening the sidebar. -->
  <div class="flex flex-row w-full min-w-0 items-center gap-x-2 flex-nowrap [contain:inline-size]">
    <div class="flex flex-row items-center gap-1.5 min-w-0 shrink">
      <button
        on:click={toggleCollapsed}
        class="flex shrink-0 items-center gap-2 font-semibold px-2 py-1 rounded transition-colors duration-250 whitespace-nowrap tabular-nums"
      ...
      <input
        ...
        title={line.name}
        class="pl-1.5 rounded-md bg-neutral-100 dark:bg-neutral-950 dark:border-neutral-700 border-[0.5px] focus:outline-none text-sm font-semibold w-24 min-w-[3.5rem] shrink-[100]"
      ...
      <select
        ...
        class="px-2 py-1 text-xs rounded border border-neutral-300 dark:border-neutral-600 bg-neutral-100 dark:bg-neutral-900 max-w-[8.5rem] min-w-[5.5rem] shrink"
      ...
      <button
        ...
        class="p-1 rounded transition-colors duration-250 shrink-0"
      >
        {#if line.locked}
          <svg
            xmlns="http://www.w3.org/2000/svg"
    ...
    <div class="flex flex-row items-center gap-2 shrink-0">
      <button
        class="shrink-0 px-2 py-1 text-xs font-semibold text-neutral-700 dark:text-neutral-200 bg-neutral-200/80 dark:bg-neutral-800/80 border border-neutral-300 dark:border-neutral-700 rounded disabled:opacity-40 disabled:cursor-not-allowed"
    ...
    <div class="flex flex-row items-center gap-1 ml-auto shrink-0">
      <button
  ...
  {#if collapsed && hitNames.length}
    <div class="pl-8 max-w-full">
      <span class={hitPillClass} title={hitTitle}>
        <span class="shrink-0">⚠ hits</span>
        <span class="truncate">{hitNames.join(", ")}</span>
      </span>
    </div>
  {/if}
  ...
  {#if !collapsed}
    ...
    <div class="flex flex-col justify-start items-start w-full">
      <!-- The collision pill shares the "Point Position" line so it never
           reflows the (already dense) path header. -->
      <div class="flex flex-row items-center justify-between gap-3 w-full min-w-0">
        <div class="font-light shrink-0">Point Position:</div>
        {#if hitNames.length}
          <span class={hitPillClass} title={hitTitle}>
            <span class="shrink-0">⚠ hits</span>
            <span class="truncate">{hitNames.join(", ")}</span>
