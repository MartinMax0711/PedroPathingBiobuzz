...
export function calculatePathTime(
  ...
  // The robot is placed facing the first sequenced path's start heading
  // (startHeadingDeg in biobuzz/transform.ts), so no turn is timed before it,
  // even when the sequence opens with a wait.
  const firstPathItem = seq.find((item) => item.kind === "path");
  const firstLine =
    firstPathItem && firstPathItem.kind === "path"
      ? lineById.get(firstPathItem.lineId)
      : undefined;
  if (firstLine?.endPoint) {
    currentHeading = getLineStartHeading(firstLine, startPoint);
  }
  ...
  seq.forEach((item) => {
    ...
    if (isFirstPath) currentHeading = requiredStartHeading;
    isFirstPath = false;
    const diff = Math.abs(
...
export function formatTime(totalSeconds: number): string {
  ...
  // Round to tenths first so 59.96 s shows as 1:00.0s, not 0:60.0s.
