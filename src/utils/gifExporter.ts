...
export async function exportAsGif(options: GifExportOptions): Promise<Blob> {
  ...
  const gif = new GIF({
    ...
    width,
    height,
    // Resolve against Vite's base so exports also work under a sub-path
    // (e.g. GitHub Pages): "./gif.worker.js" in builds, "/gif.worker.js" in dev.
...
/**
 * Rasterize an inline <svg> (e.g. the field overlay) into an image of the
 * given pixel size. The clone gets explicit width/height so browsers that need
 * an intrinsic size (Firefox) draw it; only presentation attributes survive
 * serialization, page CSS does not.
 */
export async function svgElementToImage(
  svg: SVGSVGElement,
  width: number,
  height: number,
