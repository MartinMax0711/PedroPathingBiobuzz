...
<script lang="ts">
  ...
  import BiobuzzOverlay from "./lib/components/BiobuzzOverlay.svelte";
  ...
  import { assetUrl, fieldMapUrl } from "./utils/assets";
  import { pickPathColor, withAlpha } from "./utils/draw";
  import { getLineEndHeading, getLineStartHeading } from "./utils/math";
  import { upgradeBiobuzzObstacles, uniqueShapeId } from "./utils/shapes";
  import { PP_FILE_VERSION } from "./utils/file";
  import { startHeadingDeg } from "./biobuzz/transform";
  import {
    findPathCollisions,
    type PathCollision,
  } from "./biobuzz/collision";
  ...
  function normalizeLines(input: Line[]): Line[] {
    ...
    return (input || []).map((line) => ({
      ...
      // Missing colours come from the curated palette (readable on the field).
      color: (() => {
        const color = line.color || pickPathColor(usedColors);
  ...
  const DEFAULT_FIELD_SRC = fieldMapUrl(DEFAULT_SETTINGS.fieldMap || "biobuzz.svg");
  const DEFAULT_ROBOT_SRC = assetUrl("/robot.png");
  $: fieldMapSrc =
    settings.fieldMap === "custom"
      ? assetUrl(settings.customFieldImage) || DEFAULT_FIELD_SRC
      : settings.fieldMap
        ? fieldMapUrl(settings.fieldMap)
        : DEFAULT_FIELD_SRC;
  $: robotSrc = assetUrl(settings.robotImage || "/robot.png") || DEFAULT_ROBOT_SRC;

  /**
   * One-shot <img> fallback: swap to `fallback` once, then give up, so a
   * missing fallback (e.g. a wrong base path) cannot loop on `error`.
   */
  function imageFallback(event: Event, fallback: string, what: string) {
    const img = event.currentTarget as HTMLImageElement | null;
    if (!img || img.dataset.fallbackApplied === "1") return;
    img.dataset.fallbackApplied = "1";
    console.error(`Failed to load ${what}:`, img.src);
    if (img.src !== new URL(fallback, document.baseURI).href) img.src = fallback;
  }

  function imageLoaded(event: Event) {
    const img = event.currentTarget as HTMLImageElement | null;
    if (img) delete img.dataset.fallbackApplied;
  }
  ...
  const createDefaultPathChain = (sourceLines: Line[]): PathChain => ({
    ...
    // Same colour as the default path, so Path 1 keeps BIOBUZZ yellow.
  ...
  /** The start-up project, to tell whether it is still untouched. */
  const pristineProjectKey = () =>
    JSON.stringify({
      startPoint,
  ...
  const initialProjectKey = pristineProjectKey();

  /**
   * The built-in default start is G304-legal for the default 16 × 16 robot.
   * Once the saved robot size is known, rebuild the untouched default for it
   * (e.g. an 18 × 18 robot then opens on a legal start). Ids stay the same, so
   * the sequence and chain need no changes.
   */
  function fitDefaultProjectToRobot() {
    if (isLoaded || pristineProjectKey() !== initialProjectKey) return;
    const length = settings.rWidth || DEFAULT_ROBOT_WIDTH;
    const width = settings.rHeight || DEFAULT_ROBOT_HEIGHT;
    const [fitted] = getDefaultLines(length, width);
    startPoint = getDefaultStartPoint(length, width);
    lines = lines.map((line, i) => (i === 0 ? { ...line, endPoint: fitted.endPoint } : line));
  }
  ...
  // Multiple Paths robots at their start poses: the field overlay keeps its
  // labels off them (they are drawn with the main robot's size).
  $: overlayExtraRobots =
    $activePaths.length > 0
      ? additionalPaths
          .filter((p) => p.startPoint && p.lines.length > 0)
          .map((p) => ({
            x: p.startPoint!.x,
            y: p.startPoint!.y,
            heading: startHeadingDeg(p.startPoint!, p.lines, p.sequence),
          }))
      : [];
  ...
  function getAppState(): AppState {
    return {
      ...
      shapes,
      sequence,
      // Only robot size / motion tuning: display preferences (theme, overlays,
      // field map, ...) are not part of undo history.
      settings: projectSettings(settings) as Settings,
      pathChains,
    };
  }
  ...
  function undoAction() {
    ...
    if (prev) {
      ...
      settings = mergeFileSettings(settings, prev.settings);
  ...
  function redoAction() {
    ...
    if (next) {
      ...
      settings = mergeFileSettings(settings, next.settings);
  ...
  function normalizePathChains(
    ...
    const usedColors = (sourceChains || []).map((chain) => chain?.color);
    const cleaned = (sourceChains || [])
      ...
      .map((chain, i) => ({
        ...
        color:
          chain.color ||
          (i === 0 ? DEFAULT_PATH_COLOR : pickPathColor([DEFAULT_PATH_COLOR, ...usedColors])),
        lineIds: (Array.isArray(chain.lineIds) ? chain.lineIds : []).filter((id) =>
          validLineIds.has(id),
        ),
      }));
    ...
    // Every path belongs to a chain (the code export only writes chained
    // paths): anything left out, e.g. from an older file, joins the first one.
    const chained = new Set(cleaned.flatMap((chain) => chain.lineIds));
    const orphans = sourceLines
      .map((ln) => ln.id!)
      .filter((id) => id && !chained.has(id));
    if (orphans.length) {
      cleaned[0] = { ...cleaned[0], lineIds: [...cleaned[0].lineIds, ...orphans] };
    }
  ...
  $: points = (() => {
    ...
    // Add obstacle vertices as draggable points (fixed field elements have none)
  ...
  $: shapeElements = (() => {
    ...
    shapes.forEach((shape, idx) => {
      if (shape.vertices.length >= 3) {
        ...
        // Soft tinted body with a crisp outline; fixed field elements get a
        // dashed outline so they read as "part of the field", not editable.
        shapeElement.fill = withAlpha(shape.fillColor || shape.color, 0.2);
        shapeElement.stroke = withAlpha(shape.color, 0.9);
        shapeElement.linewidth = x(shape.locked ? 0.35 : 0.45);
        shapeElement.join = "round";
        if (shape.locked) shapeElement.dashes = [x(1.2), x(0.7)];
  ...
  // Robot footprint vs. obstacles along the main path (drawn by the overlay,
  // listed in the sidebar). Multiple Paths mode hides the main path.
  let collisions: PathCollision[] = [];
  $: collisions =
    $activePaths.length === 0
      ? findPathCollisions(
          startPoint,
          ...
          settings.rHeight || DEFAULT_ROBOT_HEIGHT,
        )
      : [];

  // Overlay <svg> (rasterized into GIF frames)
  let overlaySvg: SVGSVGElement | null = null;

  // Keep the Two.js <svg> width/height attributes in sync with the canvas.
  // `fitted` only measures once, and the serialized SVG (GIF export) uses
  // these attributes, so a stale size would misalign exported paths.
  function syncRendererSize(instance: Two | undefined, w: number, h: number) {
    if (!instance || !(w > 0) || !(h > 0)) return;
    const renderer = instance.renderer as unknown as {
      width: number;
      height: number;
      setSize: (w: number, h: number) => void;
    };
    if (renderer.width !== w || renderer.height !== h) renderer.setSize(w, h);
  }
  $: syncRendererSize(two, width, height);
  ...
  onMount(() => {
    setTimeout(async () => {
      isLoaded = true;
      recordChange();
      // Turning tracking on re-runs the "unsaved" watcher once; a fresh
      // start has nothing unsaved yet.
      await tick();
      isUnsaved.set(false);
    }, 500);
  });
  onMount(async () => {
    ...
    fitDefaultProjectToRobot();
  });

  // Robot size / motion changes (Settings dialog) become their own undo step,
  // so a later Undo of a path edit does not silently revert them.
  const debouncedRecordSettings = debounce(() => recordChange(), 600);
  $: projectSettingsKey = JSON.stringify(projectSettings(settings));
  $: if (isLoaded && projectSettingsKey) debouncedRecordSettings();
  ...
  async function exportPathAsGif() {
    ...
    try {
      ...
      const scale = 0.65;
      // Make sure the serialized Two.js <svg> has the on-screen size.
      syncRendererSize(two, twoElement.clientWidth, twoElement.clientHeight);
      two.update();
      ...
      const fieldImage = await loadImage(fieldMapSrc).catch(async () => {
        return loadImage(DEFAULT_FIELD_SRC);
      });
      const robotImage = await loadImage(robotSrc).catch(async () => {
        return loadImage(DEFAULT_ROBOT_SRC);
      });

      // Field + overlay never change during the export: rasterize them once
      // at the output size and blit that for every frame.
      let background: HTMLCanvasElement | null = null;
      const buildBackground = async (w: number, h: number) => {
        const canvas = document.createElement("canvas");
        canvas.width = w;
        canvas.height = h;
        const bg = canvas.getContext("2d");
        if (!bg) return null;
        bg.drawImage(fieldImage, 0, 0, w, h);
        if (overlaySvg) {
          try {
            const overlayImage = await svgElementToImage(overlaySvg, w, h);
            bg.drawImage(overlayImage, 0, 0, w, h);
          } catch (err) {
            console.warn("GIF export: field overlay skipped", err);
          }
        }
        return canvas;
      };
      ...
      const blob = await exportAsGif({
        ...
        onDrawBackground: async (ctx, outputWidth, outputHeight) => {
          if (
            !background ||
            background.width !== outputWidth ||
            background.height !== outputHeight
          ) {
            background = await buildBackground(outputWidth, outputHeight);
          }
          if (background) {
  ...
  // Save As: a copy under a new name, in the File Manager and on disk.
  let saveAsDialogOpen = false;
  let saveAsDefaultName = "path";

  $: currentFileBaseName =
    $currentFilePath?.split(/[\\/]/).pop()?.replace(/\.pp$/i, "") || "";

  /**
   * Write the project into the browser store under `fileName` and make it the
   * open file. Replacing a different file that already has the name needs a
   * confirmation.
   */
  async function storeProjectAs(fileName: string, content: string): Promise<boolean> {
    if (
      fileName !== $currentFilePath &&
      (await browserFileStore.fileExists(fileName)) &&
      !confirm(`"${fileName}" already exists in the File Manager. Replace it?`)
    ) {
      return false;
    }
    await browserFileStore.writeFile(fileName, content);
  ...
  async function saveFileAs() {
    ...
    const suggestedBase = currentFileBaseName ? `${currentFileBaseName}_copy` : "path";

    // Chrome / Edge: the native Save dialog picks the file on disk.
    if (win.showSaveFilePicker) {
      ...
      try {
        ...
        // The user closed the dialog: nothing to do.
        if (err?.name === "AbortError") return;
        // Picker not usable here (e.g. blocked): ask for a name instead below.
        console.warn("showSaveFilePicker unavailable, asking for a name:", err);
        handle = null;
      }
      ...
      if (handle) {
        ...
        // Keep a File Manager copy under the same name; that copy is what
        // Save overwrites from now on.
        const diskName: string = handle.name || `${suggestedBase}.pp`;
        const storeName = /\.pp$/i.test(diskName)
          ? diskName
          : `${diskName.replace(/\.[^.]*$/, "")}.pp`;
        let stored = false;
        try {
          stored = await storeProjectAs(storeName, content);
        } catch (err) {
          console.warn("Failed to add the Save As copy to the File Manager:", err);
        }
        alert(
          stored
            ? `Saved to ${diskName} on your computer and as ${storeName} in the File Manager.`
            : `Saved to ${diskName} on your computer.`,
        );
        return;
      }
    }

    // Safari / Firefox (no native Save dialog): ask for a new name, then store
    // a NEW File Manager file and download it. Never overwrite the open file.
    saveAsDefaultName = suggestedBase;
    saveAsDialogOpen = true;
  }

  async function confirmSaveAs(input: string) {
    const base = input.trim().replace(/\.pp$/i, "");
    if (!base) return;
    const fileName = `${base}.pp`;
    try {
      const stored = await storeProjectAs(
        fileName,
        JSON.stringify(projectFileData(), null, 2),
      );
      if (!stored) return;
      downloadTrajectory(
        startPoint,
      ...
      alert(`Saved as ${fileName} in the File Manager and downloaded it to your computer.`);
    } catch (err) {
      console.error("Save As failed:", err);
  ...
  onMount(() => {
    ...
    const isLockedShapeElem = (id: string | null): boolean => {
      if (!id || !id.startsWith("obstacle")) return false;
      const shapeIdx = Number(id.split("-")[1]);
      return !!shapes[shapeIdx]?.locked;
    };

    const isLockedPathElem = (id: string | null): boolean => {
      if (isLockedShapeElem(id)) return true;
    ...
    two.renderer.domElement.addEventListener("mousemove", (evt: MouseEvent) => {
      ...
      if (isDown && currentElem) {
        ...
        // Keep everything on the field (with or without snapping), in 0.001 in
        inchX = Math.round(Math.max(0, Math.min(FIELD_SIZE, inchX)) * 1000) / 1000;
        inchY = Math.round(Math.max(0, Math.min(FIELD_SIZE, inchY)) * 1000) / 1000;
        ...
        if (currentElem.startsWith("obstacle-")) {
          ...
          if (!shape || shape.locked || !shape.vertices[vertexIdx]) return;
        ...
        if (
          ...
          (elem?.id.startsWith("obstacle") && !isLockedShapeElem(elem.id))
        ) {
    ...
    // Listen on window so releasing the mouse outside the field (e.g. over
    // the sidebar) still ends the drag instead of leaving it stuck.
    const endDrag = () => {
      if (!isDown) return;
      isDown = false;
      dragOffset = { x: 0, y: 0 };
    ...
    two.renderer.domElement.addEventListener("dblclick", (evt: MouseEvent) => {
      ...
      // Create a new path ending at the clicked position
      appendPath({ x: inchX, y: inchY, heading: "tangential", reverse: false });
      two.update();
    });
  ...
  document.addEventListener("keydown", function (evt) {
    ...
    if (evt.code === "Space" && (active === document.body || active === twoElement)) {
      evt.preventDefault();
      if (playing) {
        pause();
      } else {
  ...
  async function loadFile(evt: Event) {
    ...
    // Parse the upload, open it through the shared loader and, when it is a
    // valid project, cache it into the browser store (File Manager).
    loadTrajectoryFromFile(
      evt,
      (data) => {
        openProject(data, file.name, null, { cache: true });
      },
      () => {
        alert(`Could not open ${file.name}: not a valid .pp project`);
      },
    );
  ...
  /** A .pp project needs a start point and at least one path with an end point. */
  function isValidProject(data: any): boolean {
    if (!data || typeof data !== "object" || Array.isArray(data)) return false;
    const sp = data.startPoint;
    if (!sp || typeof sp !== "object" || !isFiniteNumber(sp.x) || !isFiniteNumber(sp.y)) {
      return false;
    }
    ...
    return data.lines.every(
      (ln: any) =>
        ...
        typeof ln === "object" &&
        ln.endPoint &&
        typeof ln.endPoint === "object" &&
        isFiniteNumber(ln.endPoint.x) &&
        isFiniteNumber(ln.endPoint.y),
    );
  }

  /**
   * Obstacles from a file: none (or an empty list) keeps the BIOBUZZ field
   * elements, and duplicate ids (older builds could create them) are made
   * unique so the overlay and the collision report can tell shapes apart.
   */
  function loadedShapes(input: unknown): Shape[] {
    const list = (Array.isArray(input) ? input : []).filter(
      (sh: any) => sh && typeof sh === "object" && Array.isArray(sh.vertices),
    ) as Shape[];
    if (list.length === 0) return getDefaultShapes();
    return upgradeBiobuzzObstacles(dedupeShapeIds(list));
  }

  function dedupeShapeIds(list: Shape[]): Shape[] {
    const taken = list.map((sh) => ({ id: String(sh.id ?? "") }) as Shape);
    const seen = new Set<string>();
    return list.map((sh) => {
      let id = typeof sh.id === "string" ? sh.id.trim() : String(sh.id ?? "");
      if (!id || seen.has(id)) {
        const prefix = id.replace(/-\d+$/, "") || "obstacle";
        id = uniqueShapeId(prefix, taken);
        taken.push({ id } as Shape);
      }
      seen.add(id);
      return id === sh.id ? sh : { ...sh, id };
    });
  }

  /** Sequence from a file: drop entries for missing paths, append unlisted ones. */
  function loadedSequence(input: unknown, sourceLines: Line[]): SequenceItem[] {
    const ids = new Set(sourceLines.map((ln) => ln.id!));
    const items = (Array.isArray(input) ? input : []).filter(
      (item: any) =>
        item &&
        typeof item === "object" &&
        (item.kind === "path" ? ids.has(item.lineId) : item.kind === "wait"),
    ) as SequenceItem[];
    const listed = new Set(
      items.flatMap((item) => (item.kind === "path" ? [item.lineId] : [])),
    );
    const missing = sourceLines
      .filter((ln) => !listed.has(ln.id!))
      .map((ln) => ({ kind: "path", lineId: ln.id! }) as SequenceItem);
    return [...items, ...missing];
  }

  /** Everything a .pp file stores (the same shape for every save path). */
  function projectFileData() {
    return {
      startPoint,
      lines,
      shapes,
  ...
  /**
   * The one project loader, used by the upload button and the File Manager.
   * Rejects files that are not projects (nothing changes), keeps the BIOBUZZ
   * obstacles for files without any, takes only robot/motion settings from the
   * file, repairs sequence and chains, records an undo step and leaves the
  ...
  async function openProject(
    ...
    if (!isValidProject(data)) {
      alert(`Could not open ${name}: not a valid .pp project`);
      return false;
    }
    const project = data as any;
    // Coordinates as numbers (hand-edited files sometimes quote them).
    const num = <T extends { x: unknown; y: unknown }>(p: T): T => ({
      ...p,
      x: Number(p.x),
      y: Number(p.y),
    });
    ...
    const normalizedLines = normalizeLines(
      project.lines.map((ln: any) => ({
        ...ln,
        endPoint: num(ln.endPoint),
        controlPoints: (Array.isArray(ln.controlPoints) ? ln.controlPoints : [])
          .filter((cp: any) => cp && isFiniteNumber(cp.x) && isFiniteNumber(cp.y))
          .map(num),
      })),
    );
    startPoint = num(project.startPoint);
    lines = normalizedLines;
    sequence = loadedSequence(project.sequence, normalizedLines);
    pathChains = normalizePathChains(project.pathChains, normalizedLines);
    shapes = loadedShapes(project.shapes);
    // Robot/motion settings from the file; display preferences stay local.
    settings = mergeFileSettings(settings, project.settings);
    robotWidth = settings.rWidth;
    robotHeight = settings.rHeight;
    ...
    if (options.cache) {
      try {
        await browserFileStore.writeFile(name, JSON.stringify(projectFileData()));
        openedPath = name;
      } catch (err) {
    ...
    currentFilePath.set(openedPath);

    recordChange();
    // The reactive "unsaved" watcher fires for the assignments above; clear
    // the flag once it has run so a freshly opened file is not marked "*".
    await tick();
    isUnsaved.set(false);
    two && two.update();
    return true;
  ...
  function buildOptimizationPayload(lineIndex: number) {
    ...
    return {
      waypoints,
      // This path's own headings (a path's heading lives on its end point;
      // tangential ones follow the geometry).
      start_heading_degrees: getLineStartHeading(line, startPt),
      end_heading_degrees: getLineEndHeading(line, startPt),
  ...
  function loadRobot(evt: Event) {
    const file = (evt.target as HTMLInputElement | null)?.files?.[0];
    if (file) loadRobotImage(file, () => updateRobotImageDisplay());
  }

  const round3 = (n: number) => Math.round(n * 1000) / 1000;

  /**
   * Append a path ending at `endPoint` (W hotkey, double-click on the field):
   * named like the sidebar's paths, added to the first chain so every code
   * export includes it, and drawn in that chain's colour.
   */
  function appendPath(endPoint: Point) {
    const id = `line-${Math.random().toString(36).slice(2)}`;
    const newLine: Line = {
      id,
      name: `Path ${lines.length + 1}`,
      endPoint: { ...endPoint, x: round3(endPoint.x), y: round3(endPoint.y) } as Point,
      controlPoints: [],
      color: pathChains[0]?.color || pickPathColor(lines.map((ln) => ln.color)),
      locked: false,
      waitBeforeMs: 0,
      waitAfterMs: 0,
      waitBeforeName: "",
    ...
    lines = [...lines, newLine];
    sequence = [...sequence, { kind: "path", lineId: id }];
    pathChains = pathChains.length
      ? pathChains.map((chain, i) =>
          i === 0 ? { ...chain, lineIds: [...chain.lineIds, id] } : chain,
        )
      : [createDefaultPathChain(lines)];
    recordChange();
  }
  ...
  function applyTheme(theme: "light" | "dark" | "auto") {
    ...
    // Keep the browser / installed-app title bar in step (index.html sets the
    // same colours before first paint).
    document
      .querySelectorAll('meta[name="theme-color"]')
      .forEach((meta) =>
        meta.setAttribute("content", actualTheme === "dark" ? "#171717" : "#fafafa"),
      );
  }
  ...
  // Follow system theme changes whenever the theme is "auto". Registered
  // unconditionally: settings load after mount and the user can switch to
  // "auto" at any time.
  onMount(() => {
  ...
  onMount(() => {
    ...
    const handleSaveDialog = async (event: any) => {
      ...
      try {
        ...
        const fileData = JSON.stringify(projectFileData());
    ...
    const handleDualPathSave = async (event: any) => {
      ...
      try {
        if (target === "first" && $currentFilePath) {
          ...
          // Save first path
          if ($currentFilePath) {
            const fileData1 = JSON.stringify(projectFileData());
...
<NameDialog
  bind:isOpen={saveAsDialogOpen}
  title="Save As"
  defaultValue={saveAsDefaultName}
...
>
  ...
  "
        draggable="false"
        on:load={imageLoaded}
        on:error={(e) => imageFallback(e, DEFAULT_FIELD_SRC, "field map")}
        on:dragstart={(e) => e.preventDefault()}
        on:selectstart={(e) => e.preventDefault()}
      />
      <BiobuzzOverlay
        bind:svgEl={overlaySvg}
        {settings}
        {startPoint}
        {lines}
        {sequence}
        {shapes}
        {collisions}
        showMainRobot={$activePaths.length === 0}
        extraRobots={overlayExtraRobots}
        pxPerInch={width / FIELD_SIZE}
      />
...
pointer-events: none;`}
          draggable="false"
          on:load={imageLoaded}
          on:error={(e) => imageFallback(e, DEFAULT_ROBOT_SRC, "robot image")}
          on:dragstart={(e) => e.preventDefault()}
          on:selectstart={(e) => e.preventDefault()}
        />
